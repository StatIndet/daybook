...
项目完全遵循 Obsidian 原生机制对附件进行解析与管理：

* **原生附件系统**
  * `content/` 中的所有媒体文件（图片、音频、视频、PDF）均会自动解析并原样复制至 `public/` 中。
  * 默认遵循 `.obsidian/app.json` 中配置的 `attachmentFolderPath`，完美支持 Obsidian 的相对路径、固定目录和 Vault 根目录解析。
* **远程媒体嵌入**
  * 你可以通过 `::image{url="..."}`、`::video{url="..."}`、`::audio{url="..."}`、`::pdf{url="..."}`、`::music{url="..."}` 指令跨源嵌入任何符合 HTTP 标准的远程资源。
