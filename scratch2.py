...
replacement = """  function setupNeteasePlayers() {
    ...
    players.forEach(async function(containerEl) {
      ...
      try {
        ...
        document.addEventListener('daybook:media-timeupdate', (e: any) => {
          if (e.detail.songId === id) {
            currentDuration = e.detail.duration;
            timeDiv.textContent = formatTime(e.detail.currentTime) + " / " + formatTime(e.detail.duration);
            smoothedProgress = e.detail.progress;
        ...
        function loop(t: number) {
          ...
          // Progress is strictly driven by events now, just animate the wave
          drawWave(smoothedProgress, wavePhase);
